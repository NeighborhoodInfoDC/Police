/**************************************************************************
 Adapted from: Macros/Offense_to_event.sas  (NeighborhoodInfoDC/Police)
 Original author: P. Tatian.  Autocall macro %Offense_to_event maps MPD
 crime Offense + Method description strings to the MPD two-character
 event code and the Urban Institute numeric ui_event code. The macro
 body is reproduced verbatim; a small caller exercises it against a
 sample of offense/method pairs in place of the live MPD extract.
**************************************************************************/
...
%macro Offense_to_event;
  ...
  else if offense in ( 'Adw', 'Assault W/Dangerous Weapon' ) and method='Gun' then do;
    event = '41';
    ui_event = 141;
  end;
  else if offense in ( 'Adw', 'Assault W/Dangerous Weapon' ) and method='Knife' then do;
    event = '42';
    ui_event = 142;
  end;
  else if offense in ( 'Adw', 'Assault W/Dangerous Weapon' ) and method='Club' then do;
    event = '43';
    ui_event = 143;
  end;
  else if offense in ( 'Adw', 'Assault W/Dangerous Weapon' ) and method in ('Other','Others') then do;
    event = '40';
    ui_event = 149;
  end;
  ...
  else if offense in ( 'Theft', 'Theft/Other' ) and method in ( 'One', '1', '1 Armed' ) then do;
    event = '60';
    ui_event = 160;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method in ( 'Two', '2' ) then do;
    event = '61';
    ui_event = 161;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method=:'Attemp' then do;
    event = '62';
    ui_event = 162;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method='Shoplifting' then do;
    event = '63';
    ui_event = 163;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method='B&e Vending' then do;
    event = '64';
    ui_event = 164;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method='Bicycle' then do;
    event = '6x';
    ui_event = 165;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method='F/Mail' then do;
    event = '6x';
    ui_event = 166;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method='Tags' then do;
    event = '6x';
    ui_event = 167;
  end;
  else if offense in ( 'Theft', 'Theft/Other' ) and method in ('Other','Others') then do;
    event = '59';
    ui_event = 168;
  end;
 else if offense in ( 'Theft', 'Theft/Other' ) and method in ('Gun') then do;
    event = '57';
    ui_event = 169;
  end;
 else if offense in ( 'Theft', 'Theft/Other' ) and method in ('Knife') then do;
    event = '58';
    ui_event = 169.1;
  end;
  ...
  else if offense in ( 'Stolen Auto', 'Motor Vehicle Theft' ) and method='Attempt' then do;
    event = '71';
    ui_event = 181;
  end;
  else if offense in ( 'Stolen Auto', 'Motor Vehicle Theft' ) and method in ('Other','Others') then do;
    event = '72';
    ui_event = 182;
  end;
  else if offense in ( 'Stolen Auto', 'Motor Vehicle Theft' ) and method in ('Gun') then do;
    event = '72';
    ui_event = 183;
  end;
  else if offense in ( 'Stolen Auto', 'Motor Vehicle Theft' ) and method in ('Knife') then do;
    event = '72';
    ui_event = 184;
  end;
  ...
  label
    ...
    ui_event = 'Urban event code (type of crime, pts. 1 & 2)';
    
  format event $tcpevt. ui_event uievent.;

%mend Offense_to_event;

** Caller: sample MPD offense/method pairs in place of the live extract **;

data crimes_raw;
  length offense $ 20 method $ 20;
  infile datalines dsd dlm='|';
  input offense $ method $;
  datalines;
HOMICIDE|GUN
HOMICIDE|KNIFE
...
proc print data=crimes_coded noobs;
  var offense method event ui_event;
  title 'MPD offense/method recoded to MPD + UI event codes';
run;

proc freq data=crimes_coded;
  tables ui_event / missing;
  title 'Distribution of mapped UI event codes';
run;
